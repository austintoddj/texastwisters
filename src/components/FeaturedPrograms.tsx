      ...
      <div className="flex flex-col justify-between">
        ...
        <div className="mt-8">
          <Button href={`/programs/${program.slug}`} variant="accent" size="sm">
            Our{' '}
            <span className="lowercase">&nbsp;{program.data.name}&nbsp;</span>{' '}
            program
            <Icon
...
export const FeaturedPrograms = async () => {
  const allPrograms = await getAllItems<ProgramData>('programs')
  const featuredPrograms = allPrograms
    .filter(program => program.data.featured && Boolean(program.data.hero))
    .slice(0, 3)
